...
当用户输入 `/custom-rules-help` 或包含以下关键词时，进入外置规则帮助模式，而非代码审查模式：

**关键词**："写外置规则"、"添加外置规则"、"自定义规则"、"怎么加规则"、"禁用规则"、"override 规则"

**三级响应**：

| 级别 | 触发条件 | 行为 |
|------|---------|------|
| **L1 快速参考** | 用户问"overrides.md 格式是什么"等简单格式问题 | 内联输出格式模板（不读取文件） |
| **L2 完整教程** | 用户说 `/custom-rules-help`、"/custom-rules-help"请求或问"怎么写外置规则" | 读取 `references/external-rules-guide.md` 并输出全文 |
| **L3 交互生成** | 用户说"帮我添加一条外置规则"或直接描述需求如"禁用 MEM-03" | 逐步引导用户确认规则内容，完成后写入对应的 `.android-performance-cr/` 文件 |

**L3 交互引导模板**：

1. 确认操作类型（disable / override / add）
2. 询问规则 ID（如 MEM-03 / ST-01 / CUSTOM-01）
3. 若为 override：询问新等级（P0/P1/P2）+ 原因
4. 若为 add：按七列格式依次询问（检查项 / 反例 / 正例 / 判定 / 参考工具）
5. 确认后，判断应写入 `overrides.md`（4 列操作）还是 `custom-rules.md`（7 列规则）
6. 生成完整文件内容，提示用户保存到 `<项目根>/.android-performance-cr/`
