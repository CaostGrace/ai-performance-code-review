...
Skill SHALL 通过扫描文件内容中的 `@Composable` 或 `Composable` 关键字识别 Compose 体系文件。
...
对标记为 Compose 的文件 SHALL 不应用 View 专用规则。对标记为 View 的文件 SHALL 不应用 Compose 专用规则。

#### Scenario: Compose 文件不加载 View 规则
- **WHEN** 变更文件为纯 Compose 文件
- **THEN** `ui-view.md` SHALL 不加载

#### Scenario: View 文件不加载 Compose 规则
- **WHEN** 变更文件为纯 View 文件
- **THEN** `ui-compose.md` SHALL 不加载
