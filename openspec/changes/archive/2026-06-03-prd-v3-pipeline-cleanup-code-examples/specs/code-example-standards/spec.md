...
代码片段 SHALL 来源于 Android 官方文档、真实生产事故复盘或 Kotlin 编码规范。每条规则的代码片段优先级：① Android 官方文档 → ② 真实生产事故（掘金/CSDN/proandroiddev）→ ③ 编码规范推荐写法。

#### Scenario: 官方文档优先
- **WHEN** Android 官方文档（developer.android.com）中存在对应反模式/最佳实践示例
...
所有代码片段 SHALL 使用标准 Kotlin 语法，避免使用 Java-only API（除非规则明确涉及 Java 互操作场景，如 JNI）。代码片段 SHALL 可独立阅读，不依赖外部类定义。
...
全部 12 个维度（10.1 到 10.12）的 81 条规则 SHALL 均完成代码片段增强。已完成增强的规则（如 TH-05、MEM-02、MEM-06、WV-03、UI-01）SHALL 保持现有代码不变。
