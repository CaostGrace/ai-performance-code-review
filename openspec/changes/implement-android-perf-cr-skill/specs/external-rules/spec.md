...
外置规则 SHALL 随 IDE 工作区切换自动热加载，无需手动 reload。
...
`overrides.md` SHALL 支持三种覆盖操作：`disable`（禁用规则）、`override`（改变等级）、`add`（追加新规则）。`override` 操作 SHALL 仅改变规则等级，不修改判定条件或其他属性。

#### Scenario: `overrides.md` 中包含 `| override | ST-01 | P1 | ...` 条目时，ST-01 按 P1 等级审查，不再作为 P0 阻塞合入。
- **WHEN** overrides.md override ST-01 为 P1
- **THEN** ST-01 SHALL 按 P1 审查，检查项和判定条件保持不变

#### Scenario: `custom-rules.md` 中包含完整七列新规则时，该规则与内置规则同权重参与审查，ID 冲突时以项目规则优先。
...
Phase 0 阶段，外置规则与内置规则之间的重叠 SHALL 不做自动去重处理。双报通过 Reviewer 人工标记 `#false-positive` 回注。
...
`override` 操作 SHALL 仅改变规则等级，不支持修改判定条件。阈值定制需求 SHALL 走 `custom-rules.md` 的 `add` 操作新增独立规则。

#### Scenario: 阈值定制
- **WHEN** 团队需要"启动初始化耗时 < 200ms 不算 P0"的自定义判定
- **THEN** SHALL 通过 `custom-rules.md` 新增 CUSTOM 规则，而非 override ST-01
