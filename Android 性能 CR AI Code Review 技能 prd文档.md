...
| 标准 | 目标 | 验证方式 |
|------|------|----------|
| 报告可产出 | Skill 对 `git diff` 产出符合 §3.3 模板的 Markdown 审查报告 | 本地触发 `/ @android-performance-cr`，验证产出 `android-performance-cr-report.md` |
| 规则全覆盖 | 审查报告覆盖 §9 全部 12 维度 | 检查报告中「已查维度」字段 |
| P0 可识别 | 能识别代码中的明显主线程 IO、泄漏等 P0 反模式 | 使用 §9 反例代码片段构造测试 diff，验证产出 P0 发现项 |

> 长期成功指标（CR 覆盖率、问题前置率、审查一致性、误报率）已移至 [§10.2 路线图](#102-自动化阶段路线图)。
...
| **扫描范围** | **全量**：§9 全部维度均执行；Author 可声明仅审 §6 速查维度（须在输出中写明「缩减范围」） |
...
| 本 PRD §9 | 规则权威来源；Skill/流水线均引用同一规则集 |

### 3.5 规则加载机制：内置规则 + 外置规则

Skill 采用 **双层规则模型**，兼顾通用性与项目定制：

**内置规则**：Skill 内部预置本 PRD §9 的全部规则（约 40+ 条），开箱即用。
...
flowchart TD
    ...
    Check2 -->|"否"| Default["加载内置规则集<br/>§9 标准规则"]
    Load2 --> Default
    Default --> Done["开始审查"]
```
...
> 审查超时、并发限制、大 PR 降级等 SLA 相关策略已移至 [§10 后期展望](#10-后期展望)。

> 质量保障与反馈机制（误报反馈、漏报收集、审查校准、一致性度量）已移至 [§10.3 后期展望](#103-质量保障与反馈机制)。
...
- 规则解释以本文档 §9 为准；歧义由性能组 + Team Leader 裁定。

- 业务强需求无法满足时：降级为 P1 + Issue + 线上监控。

- AI 与人工结论冲突：以人工为准；争议裁定结果须记录（规则 ID、裁定理由、最终等级、裁定人、日期），供后续规则迭代参考（详见 [§10.3](#103-质量保障与反馈机制)）。
...
> 已决项已体现在正文中；待排期与讨论中事项已移至 [§10.4 待决策开放项](#104-待决策开放项)。
