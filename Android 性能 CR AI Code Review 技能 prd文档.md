...
| 关联产物 | Agent Skill（`android-performance-cr`），平台无关，按通用 Skill 规范实现 |
...
Android 应用在启动速度、流畅度、内存、电量与稳定性上的体验，高度依赖日常迭代中的代码质量。性能问题往往在合入后才发现，修复成本高。需要在 **Code Review（CR）** 阶段建立可执行、可度量的检查规则，并封装为 **AI Agent Skill**，供 AI Code Reviewer 在 PR 审查时稳定调用，将性能风险前置拦截。Skill 按通用 Agent Skill 规范实现，不绑定特定 IDE 平台。
...
| 标准 | 目标 | 验证方式 |
|------|------|----------|
| 报告可产出 | Skill 对 `git diff` 或 @ 文件产出符合 §3.3 模板的 Markdown 审查报告 | 本地触发审查，验证产出 `./android-performance-cr-report.md` |
| 规则全覆盖 | 审查报告覆盖 §9 全部 12 维度 | 检查报告中「已查维度」字段 |
| P0 可识别 | 能识别代码中的明显主线程 IO、泄漏等 P0 反模式 | 使用 §9 反例代码片段构造测试 diff，验证产出 P0 发现项 |
| 正反例测试通过 | 全部 P0 规则反例能触发对应规则 ID，正例不触发误报 | 13 条 P0 规则的反例/正例代码作为冒烟测试输入 |
...
| 重构/架构 PR | 启动链、生命周期、线程模型（§9.2、§9.8、§9.1） | 同上 |
| 依赖升级 PR | 包体、启动、主线程（§9.9、§9.2） | 同上 |
| 仅文案/颜色 | §9.3（若动布局）、§9.9（若新增资源） | 流水线仍全量扫描；本地可按 §6 速查缩减 |

---

## 3. AI Skill 产物定义

本 PRD 的规则清单将落地为 **Agent Skill**。Skill 是审查时的「操作手册」，不是应用代码。按通用 Skill 规范实现，不绑定特定 IDE 平台（Cursor、CodeBuddy 等）。

### 3.1 触发方式

审查能力通过 **Skill + Markdown 输出**，由开发者在 IDE 中触发；**不绑定** GitLab、GitHub 等特定平台的评论或 API 形态。

| 属性 | 说明 |
|------|------|
| **触发方式** | 开发者在 IDE 中调用 Skill，支持三种输入模式：① 自动 `git diff`（未 @ 任何文件时）；② 手动 @ 指定文件/目录；③ commit hash（7-40 位 hex，通过 `git show <hash>` 审查已提交变更） |
| **扫描范围** | 通过**两层动态路由**自动确定：第一层路径/关键字粗筛 + 第二层 P0 必查兜底（L0 永远注入 + L1 按文件语言注入） |
...
| 输入 | 说明 |
|------|------|
| 代码变更集 | `git diff`（未提交）、`git show <hash>`（已提交 commit）、或用户 @ 指定文件/目录 |
...
- **扫描范围**：full（§9 全量）| reduced（注明依据 §6 条目）
...
Skill 采用**多文件结构**：1 个 SKILL.md 入口 + 14 个按维度拆分的规则文件 + 1 个工具速查卡。
...
│   ├── main-thread.md        # §9.1 主线程与 ANR
│   ├── startup.md            # §9.2 启动与初始化
│   ├── ui-view.md            # §9.3 UI 渲染（View 体系）
│   ├── ui-compose.md         # §9.3.A Compose 专项
│   ├── memory.md             # §9.4 内存
│   ├── network.md            # §9.5 网络
│   ├── disk-io.md            # §9.6 磁盘与 I/O
│   ├── background.md         # §9.7 后台、电量与系统合规
│   ├── threading.md          # §9.8 线程与协程
│   ├── package.md            # §9.9 APK、资源与依赖
...
Skill 采用 **三层规则模型**，兼顾审查效率与项目定制：

**第一层 — 动态路由粗筛**：Skill 启动后对每个变更文件执行关键字扫描（grep），基于 §6.1 路径启发式和 §6.2 影响半径确定应加载的维度规则文件。同时执行文件级语言体系识别（`@Composable` → Compose 体系、`RecyclerView`/`findViewById` → View 体系），按文件分别加载对应规则子集。

**第二层 — P0 必查兜底**：无论路由结果，始终加载 `_must-check.md`。分两级：
- **L0（永远注入）**：MEM-01、MEM-02、TH-05 — 无论改什么代码都可能引入的致命问题
- **L1（按文件语言注入）**：MT-01/02/03/06、BG-01/05、ST-01、IO-01 — 仅在非资源文件变更时注入

**外置规则**：项目可通过在仓库根路径放置规则文件，在审查时自动合并。外置规则随 IDE 工作区切换自动热加载，无需手动 reload。Skill 启动时按以下**发现顺序**加载（后加载的同 ID 规则覆盖内置）：
...
| 3（默认） | Skill 内置规则集 | §9 标准规则，作为兜底 |

```mermaid
flowchart TD
    Start["Skill 启动"] --> Scan["扫描变更文件<br/>关键字 + 语言体系"]
    Scan --> Route["第一层路由<br/>确定维度规则文件"]
    Route --> Safety["注入 _must-check.md<br/>L0 永远 + L1 条件"]
    Safety --> Check1{"项目根存在<br/>overrides.md?"}
...
**外置规则文件格式**：与 §9 表格同构（Markdown 表格）；每条规则至少包含 `ID | 等级 | 检查项 | 反例 | 正例 | 判定 | 参考工具`。未指定 ID 的规则将被跳过。
...
**外置规则编写指南**：详见 Skill 内置教程 `references/external-rules-guide.md`，包含三种操作的正反例、七列详解、3 个真实场景示例和常见错误 FAQ。开发者可通过 Skill 内输入 `/custom-rules-help` 或"怎么写外置规则"获取交互式引导。
...
flowchart LR
    Issue["发现项"] --> Match{"匹配度?"}
    Match -->|"精确匹配反例<br/>+ 代码位置"| P0["按原等级报告<br/>P0/P1/P2"]
    Match -->|"模式可疑<br/>不确定"| Downgrade["降一级报告<br/>P0→P1, P1→P2<br/>注明需人工确认"]
    Match -->|"架构担忧<br/>无代码证据"| Manual["写入需人工<br/>不报告等级"]
...
- **精确匹配**：代码与反例模式高度一致且可定位到具体行号 → 按表定级
- **模式可疑**：代码与反例相似但无法确定是否真正触发性能问题（如量级未知、上下文不完整）→ 降级报告
- **架构担忧**：仅架构层面「可能」变慢且无代码证据 → 写入需人工
...
1. 确定输入范围（自动 `git diff` 或用户 @ 指定文件）。

2. 执行**两层动态路由**：第一层路径/关键字粗筛确定维度规则文件 + 文件级语言体系识别；第二层注入 P0 必查规则集（L0 永远 + L1 按文件语言）。

3. 加载规则文件后逐维度审查，按**三重判定逻辑**定级：精确匹配→原等级 / 模式可疑→降级 / 架构担忧→需人工。

4. 无法从静态 diff 判断的项（含 §3.7 盲区类型）写入「未覆盖/需人工」，不得臆造 P0。

5. **P0 清零策略**：强制要求重跑 Skill 才能清 P0，不接受纯人工备注声明。

6. 例外合入 P0：须 Team Leader 批准，PR 记录原因、偿还计划、监控项。

7. **报告完整性检查**：产出前确认 `合入建议` 字段与 P0 状态一致。报告固定路径 `./android-performance-cr-report.md`，每次覆盖。
...
| v2.0 | 2026-06-03 | — | 全面覆盖：§11 兜底规则转正、Compose 专项、§9.12 架构、§7.4 工具速查卡、真实案例；规则总数 64→81 |
| v3.0 | 2026-06-03 | — | 激进清理流水线至 §13；81 条规则全部补全可编译 Kotlin 反例/正例；新增 Compose Compiler Metrics 条目 |
| v3.1 | 2026-06-05 | — | grill-me 设计澄清：多文件结构、两层动态路由、P0 必查兜底（L0+L1）、三重判定逻辑、文件级语言体系识别、双输入模式、外置规则热加载、固定报告路径、正反例即测试用例、Phase 0 格式校验、OBS-01 策略、P0 清零策略、平台无关实现 |
| v3.2 | 2026-06-06 | — | 新增外置规则编写教程（references/external-rules-guide.md），SKILL.md 新增 `/custom-rules-help` 伪命令和三级响应模式 |
| v3.3 | 2026-06-06 | — | 新增 commit hash 输入模式，支持通过 `git show <hash>` 审查已提交变更 |
...
> 本章为预留内容，将在 **Phase 2-4** 阶段实施。当前阶段仅聚焦 §9 规则与 Skill 审查流程（Phase 0：Skill Markdown 审查报告输出）。
...
| 通道 A | 流水线 输入 diff + 描述，输出固定文件名；**全量 §9** |
...
| 阶段 | 内容 | 对应规则示例 | 完成标准 |
|------|------|----------------|----------|
| Phase 0 | 生成 Skill，共用 §3.3 Markdown | 全文 | Skill 可本地产出 Markdown，覆盖 12 维度，能识别 P0 反模式 |
| Phase 1 | Lint/Detekt 自定义 | MT-01、MT-02、MEM-02 部分模式 | ≥ 80% P0 规则存在对应 Lint 检查 |
| Phase 2 | CI 解析 Markdown 中 P0 行阻断合入 | 与 §3.3 表格列 `等级` = P0 且未关闭 | ≥ 3 个 PR 成功触发阻断/放行；CR 覆盖率 ≥ 80% |
| Phase 3 | 指标看板聚合规则 ID | 从制品 `android-performance-cr-report.md` 抽取 | 看板展示 CR 覆盖率、P0 拦截率、误报率趋势；问题前置率较基线提升；审查一致性 ≥ 70%；误报率 < 15% |
| Phase 4（预留） | AI 模型微调/精调 | P0 误报率高于阈值（见 §10.3.1）的规则 | 误报率连续 4 周 < 15% |
...
- 流程：性能组每周审查线上新增性能故障 → 判断是否可用静态规则覆盖 → 归类到 §9 对应维度 → 增补规则或修改判定条件。
...
| # | 问题 | 当前建议 | 状态 |
|---|------|----------|------|
| 1 | 多模块/多进程项目是否拆分审查报告？ | 建议单份报告涵盖全项目；若模块独立上线，可按模块生成独立报告 | **已决议（v3.1）**：单份报告，固定路径 |
| 2 | 外置规则的热更新机制？ | 工作区切换自动热加载；Phase 0 不做自动去重 | **已决议（v3.1）** |
| 3 | 审查耗时/成本的监控？ | Phase 3 看板纳入；门禁阶段加入 API 调用量预警 | 待实现 |
| 4 | P0 误报与量级判定？ | Phase 0 先按一律报告+人工标记 `#false-positive` 跑数据，积累 2-4 周后补量级判定 | **已决议（v3.1）** |
| 5 | 路由漏报 P0 的兜底？ | L0 永远注入 + L1 按文件语言注入，双重兜底 | **已决议（v3.1）** |
| 6 | OBS-01 阻塞策略？ | Phase 0 不阻塞，AI 自动追加提示 | **已决议（v3.1）** |
| 7 | 输出格式不稳定处理？ | Phase 0 强约束 prompt + 格式校验脚本；Phase 2 若不稳定率 > 5% 引入 JSON 中间格式 | **已决议（v3.1）** |
