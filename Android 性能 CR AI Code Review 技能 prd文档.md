...
> 流水线相关的 SLA、并发、大 PR 策略已移至 [§13 后期展望](#13-后期展望流水线集成与-ci-自动化)。
...
> Phase 1-4 的 Lint 自定义规则、CI 门禁、看板聚合、模型微调等流水线相关内容已移至 [§13 后期展望](#13-后期展望流水线集成与-ci-自动化)。当前阶段聚焦 Skill 审查规则与流程。

当前进展：**Phase 0 — Skill Markdown 审查报告输出**。
...
| 4 | 后续流水线 Job 调用方式？ | 后续 Phase 阶段考量，见 §13 后期展望 | 待排期 |
