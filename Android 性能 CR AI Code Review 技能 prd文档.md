...
| Compose 新屏 | 10.1、10.3（含 10.3.A Compose 专项全部 CP- 规则）、10.4、10.8 | 全量 | 可缩减 |
| 模块依赖/架构变更 | 10.12、10.9、10.2 | 全量 | 可缩减 |
| 跨进程/多进程 | 10.12（ARCH-04）、10.7 | 全量 | 可缩减 |
...
> Phase 2-4 的 CI 门禁、看板、模型微调等流水线相关内容已移至 [§11 后期处理：流水线与 CI](#11-后期处理流水线与-ci)。当前阶段（Phase 0）聚焦 Skill 审查规则与流程。

| 阶段 | 内容 | 说明 |
|------|------|------|
| Phase 0 | 生成 Skill（通道 B），共用 §3.3 Markdown | **当前阶段**：通道 B 可本地产出 Markdown |
| Phase 1 | Lint/Detekt 自定义 | MT-01、MT-02、MEM-02 部分模式 → ≥ 80% P0 规则存在对应 Lint 检查 |
| Phase 2-4 | CI 门禁 / 看板 / 模型微调 | 见 §11 |

### 7.3 本地 Agent 使用指南

通道 B（本地 Agent）的审查流程与输出格式与通道 A 一致（§3.3）。开发者可在 IDE 中通过 `@android-performance-cr` 挂载本 Skill 进行预审。流水线门禁相关内容见 §11。
...
| **Compose Layout Inspector** | Android Studio → View → Tool Windows → Layout Inspector → 选择 Compose 进程 | 运行 App → 点击 Compose 组件 → 右侧面板查看 Recomposition Count | ① **重组计数**：数字越大=该组件被重组越频繁 → 性能瓶颈<br>② **跳过次数**：标记为 "skipped" 的组件表示未实际重组 → 稳定状态<br>③ **参数稳定性**：标记为 "unstable" 的参数 → 需加 `@Stable` 注解 |
...
| v2.0 | 2026-06-03 | — | 全面覆盖：§11 兜底规则转正（6 条）、Compose 专项子维度（CP-01~08）、新建 §10.12 架构模块化维度（ARCH-01~04）、§7.4 工具速查卡（9 工具）、真实错误案例增补；流水线内容后移至 §11；规则总数 64→83 |
...
| ST-07 | P1 | 启动路径改动需检查 Baseline Profile | 启动关键路径代码变更但 `src/main/baseline-prof.txt` 缺失或未更新 | 使用 Macrobenchmark 生成 `baseline-prof.txt`，通过 `ProfileInstaller` 携带 | 冷启动时间无 AOT 优化，TTID/TTFD 明显变差 | Macrobenchmark、ProfileInstaller |
...
| ID | 等级 | 检查项 | 反例 | 正例 | 判定 | 参考工具 |
|----|------|--------|------|------|------|----------|
| UI-01 | P0 | 列表必须使用 ViewHolder/等价复用 | ListView 不复用、`RecyclerView` 每次 `new View` → **真实案例**：Adapter 在 `onCreateViewHolder` 中每次 `LayoutInflater.inflate` + `new`，滑动 100 个 item 创建 100 个 View，内存暴涨+掉帧 | 标准 Adapter + ViewHolder/`RecyclerView` 复用；Compose 中 `LazyColumn` 自带复用 | 滑动掉帧、内存涨 | Layout Inspector |
...
| UI-09 | P1 | RecyclerView 大列表使用 DiffUtil 差分更新 | `adapter.notifyDataSetChanged()` 全局刷新导致所有 item 重绑、动画丢失 | `ListAdapter` + `DiffUtil.ItemCallback` 自动差分计算，仅刷新变更项 | 列表数据更新时掉帧、无条目动画 | Systrace、Layout Inspector |
...
| ID | 等级 | 检查项 | 反例 | 正例 | 判定 | 参考工具 |
|----|------|--------|------|------|------|----------|
| CP-01 | P1 | LazyColumn/LazyRow key 必须稳定唯一 | `LazyColumn { items(list) { Text(it.name) } }` 用 index 做隐式 key，增删时全量重组 | `LazyColumn { items(list, key = { it.id }) { Text(it.name) } }` 明确稳定 key | key 不唯一或频繁变化导致 Diff 低效 | Compose Layout Inspector |
| CP-02 | P1 | 数据类标注 @Stable / @Immutable 注解 | `data class User(val name: String)` 未标注，Compose 认为不稳定，全树重组 | `@Immutable data class User(val name: String)` 或 `@Stable` 声明稳定性契约 | 数据类被频繁重组 | Compose Compiler Metrics |
| CP-03 | P1 | LaunchedEffect key 必须稳定且与内部依赖一致 | `LaunchedEffect(Unit) { loadData(userId) }` key 不随 userId 变，协程遗漏重启 | `LaunchedEffect(userId) { loadData(userId) }` key 与内部使用变量一致 | 协程未在依赖变更时重启，或每次重组都重启 | Compose Layout Inspector |
| CP-04 | P1 | 频繁变化的状态用 lambda 版 Modifier | `Modifier.offset(scrollState.value)` 在组合阶段读取滚动状态导致频繁重组 | `Modifier.offset { IntOffset(0, scrollState.value) }` 延迟到布局阶段读取 | Systrace 显示频繁重组帧 | Systrace、Compose Layout Inspector |
| CP-05 | P1 | CompositionLocal 提供范围限制在最小子树 | `CompositionLocalProvider(LocalX provides value) { EntireScreen() }` 全屏提供 | 将 Provider 限制到实际需要的子树，避免值变更触发全树重组 | `CompositionLocal` 值变更时重组范围过大 | Compose Layout Inspector |
| CP-06 | P1 | Composable lambda 参数使用 remember 缓存 | `Button(onClick = { doSomething(item) })` 每次重组分配新 lambda 对象 | `val onClick = remember(item) { { doSomething(item) } }` 缓存 lambda | 子组件因 lambda 引用变化频繁重组 | Compose Compiler Metrics |
| CP-07 | P2 | 避免在频繁重组区域使用 SubcomposeLayout | `SubcomposeLayout` 嵌套在 LazyColumn item 或高频重组 Composable 中 | 使用标准布局替代；仅必需时使用，且限制嵌套深度 | 每次重组都触发子组合，性能开销大 | Compose Layout Inspector |
| CP-08 | P1 | 派生状态使用 derivedStateOf 而非直接计算 | `val filtered = items.filter { it.active }` 每次重组重新计算并触发子组件重组 | `val filtered by remember { derivedStateOf { items.filter { it.active } } }` 仅依赖变更时计算 | 基于 State 的计算触发多余重组 | Compose Layout Inspector |
...
| MEM-02 | P0 | 监听/订阅/协程必须随生命周期取消 | 未 `removeListener`、`GlobalScope.launch { updateUI(networkCall()) }` Activity 销毁后协程仍持有 View 引用 → **真实案例**：`GlobalScope.launch` 启动的协程在 Activity finish() 后仍执行 `textView.text = data` 导致 NPE 和内存泄漏 | `lifecycleScope.launch { ... }`（Activity/Fragment）、`viewModelScope.launch { ... }`（ViewModel）、`DisposableEffect`（Compose）自动管理生命周期 | 退出页面后仍回调或持有 UI 引用 | LeakCanary、Lint |
...
| MEM-06 | P0 | 禁止 Handler / 匿名内部类持有 Activity | `Handler(Looper.getMainLooper()) { ... }` 匿名对象未移除、内部类隐式持有 → **真实案例**：`class MyActivity { val handler = Handler { handleMessage(it) } }` 延迟消息在 Activity 销毁后执行，LeakCanary 报告 Activity 泄漏 | 静态内部类 + `WeakReference<Activity>`、`LifecycleObserver` 自动清理、`Handler.removeCallbacksAndMessages(null)` 在 `onDestroy` | 退出 Activity 后仍可回调 | LeakCanary |
...
| MEM-08 | P1 | 大图按需降采样，优先硬件位图 | 原图全分辨率载入、未使用 `Bitmap.Config.HARDWARE` 导致 GPU 上传/下载开销 | 按目标尺寸 `inSampleSize` 降采样；Android O+ 优先使用 `Bitmap.Config.HARDWARE` 减少像素拷贝 | 内存峰值异常、渲染卡顿、OOM | Memory Profiler、GPU 渲染分析 |
...
| TH-05 | P0 | 禁止 `runBlocking` 在主线程或任意可能阻塞主线程的路径 | `runBlocking { delay(1000) }`、`runBlocking` 包裹网络请求；**真实案例**：`runBlocking { apiService.fetchData() }` 在 Activity.onCreate 调用导致 ANR（线上事故归因） | `CoroutineScope.launch` + 挂起、`withContext` | 主线程阻塞，直接导致 ANR | Lint（协程） |
...
| PKG-06 | P1 | Native 库满足 16 KB 页大小对齐 | Native `.so` 未使用 16 KB ELF 对齐编译（`-Wl,-z,max-page-size=16384`），Google Play 拒绝上架 | NDK r27+ 默认对齐；AGP 8.5+ 自动处理；手动指定 `CMAKE_SHARED_LINKER_FLAGS` | Google Play 拒绝上架或运行崩溃（Android 15+ 16 KB 设备） | APK Analyzer、`readelf -l` |
...
| WV-03 | P1 | WebView 页面正确销毁 | 未调 `destroy()` 或 Activity `onDestroy` 中遗漏 → **真实案例**：WebView 未在 `onDestroy` 执行 `webView.destroy()`，页面退出后 Native 内存残留 50-200MB，多次进出后 OOM | 独立进程 WebView 主动 `removeAllViews()` + `destroy()`；或 `onDestroy` 中完整清理：`webView.stopLoading(); webView.loadUrl("about:blank"); webView.destroy()` | 页面退出后 Native 内存未降 | Memory Profiler |
...
| ID | 等级 | 检查项 | 反例 | 正例 | 判定 | 参考工具 |
|----|------|--------|------|------|------|----------|
| ARCH-01 | P2 | 跨模块避免大对象序列化开销 | 跨模块接口传递 > 100KB 的 Parcelable/Serializable 大对象，每次调用完整序列化 | 提炼轻量数据对象（传输关键 ID+状态），大对象通过本地缓存/数据库共享 | Parcelable 类字段 > 20 个且跨模块频繁调用 | APK Analyzer（类结构分析） |
| ARCH-02 | P2 | 避免模块间循环依赖 | `implementation project(":feature-b")` 且 `:feature-b` 已依赖当前模块，形成 `A→B→A` | 提取公共接口到 `:base` / `:foundation` 模块，依赖倒置 | `build.gradle(.kts)` 中 project() 声明构成环 | Gradle `moduleDependencyReport` |
| ARCH-03 | P1 | Dynamic Feature 按需加载，避免启动阻塞 | `SplitInstallManager.startInstall()` 在 `Application.onCreate` / 首屏 Activity 中同步调用 | 使用时按需触发加载（Activity 跳转前），配合 `SplitCompat.install` | 冷启动或主流程阻塞 | Systrace（启动阶段）、APK Analyzer |
| ARCH-04 | P1 | 多进程避免高频 Binder 通信 | 同一帧内多次 `ContentProvider` 跨进程调用、大体积 Binder 数据传输 | 批量 RPC、使用 `SharedMemory` / `MMKV` 进程内缓存、合并调用 | Binder 线程池满载或主线程 Binder 调用耗时 > 5ms | Systrace（Binder 调用轨道） |

---

## 11. 后期处理：流水线与 CI

> 本章为预留内容，将在 **Phase 2-4** 阶段由流水线团队实施。当前阶段仅聚焦 §10 规则与 Skill 审查流程。
